...
def ComprehensiveContactInfo(site):  # contact information
    ...
    for results in search(
            "site:" + site + " "
            "intitle:@gmail.com | Contact"
    ):  # Searches for webpages with gmail.com and contact
        counter = counter + 1
...
def ComprehensiveunsecureDomainsofSites(site):
    ...
    for results in search("site:" + site +
                          " "  # Websites with http instead of https 
...
def ComprehensiveShodanAdvancedSearch(site):
    ...
    )  # Includes the google dorking command for shodan search in the text file
...
def ComprehensivelookUpBugBountyDB(site):
    ...
    )  # Include link to the results of a search done on openbugbounty
...
def searchFilesOnSite(site, fileType):  # search files on site
    ...
    for results in search("https://google.com/search?q=" + site + " " +
        ...
        if request >= 5:  # Limiting console output
            break
        data = (counter, results)
        time.sleep(0.1)
...
def lookUpOpenPorts(site):  # Function for open ports in shodan search
    ...
    webbrowser.open_new("https://www.shodan.io/search/facet?query=" + site +
                        "&facet=port")  # Display results in a browser tab
...
def lookUpBugBountyDB(site):  # Function for OpenBugBounty
    ...
    webbrowser.open_new("https://www.openbugbounty.org//search/?search=" +
                        site +
                        "&type=host")  # Display results in a browser tab
    ...
    try:  # Infinite loop if invalid characters are chosen
        ...
        while True:  # Another loop to when the correct input are taken
            if userInput == 1:  # Search  files on site
                ...
                fileType = input(
                    "Enter files you want to search: ")  # Files to search
                searchFilesOnSite(site, fileType)
                menu()
                userInput = int(
                    input("Choose another option or enter 0 to quit: "))
                ...
                userSpecific = input(
                    "Specific word or words you want to search: ")
                userSpecific = userSpecific.replace(" ", "+")
                ...
                print(
                    "Comprehensive Search includes: contact info, http protocol,"
                    +
                ...
                check = os.path.exists(
                    userFile)  # Checks if the file name already exists
                ...
                ComprehensiveContactInfo(
                    site
                )  # Calls out the functions and prints the results in the comprehensive search text file
